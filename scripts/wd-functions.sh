...
if $using_git && test -z "$CHECKED_GIT_HOOKS"; then
    ...
    taobao_domains_re='(taobao\.(com|org)|alibaba-inc\.com|aliyun-inc\.com)'
    kerncvs_re='svn.simba(\.taobao\.com)?'
    if (echo $EMAIL; hostname -f) | grep -Eiq "[@.]$taobao_domains_re\\>" || \
        git config remote.origin.url | grep -Eiq "\\<$kerncvs_re:"; then
        # only warn when used in Taobao.com
        if ! git var GIT_COMMITTER_IDENT | grep -Eiq "@$taobao_domains_re>"; then
            echo "WARNING: You should set your Taobao.com email address in git. (@Alibaba or @Aliyun are also fine)"  >&2
            echo "WARNING: E.g. by running 'git config --global user.email <your login>@taobao.com'" >&2
        fi
    fi
fi
